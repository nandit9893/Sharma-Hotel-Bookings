...
const HotelItem = ({ hotel }) => {
  ...
  return (
    <div className="my-2 flex relative gap-5 bg-white p-2 w-full"> 
      <div className="flex gap-1 w-full">
        <div className="flex relative overflow-hidden">
          <img src={mainImage} alt="Hotel Thumbnail" className="w-80 h-72 object-cover cursor-pointer transition-transform duration-300 ease-in-out hover:scale-105"/>
        ...
        <div className="grid grid-cols-2 gap-1">
        {
          hotel.imageURLs.slice(0, 8).map((url, index) => (
              <img src={url} alt={`Hotel ${index}`} key={index} className="w-14 h-16 object-cover cursor-pointer" onClick={() => setMainImage(url)}/>
          ))
        }
        </div>
      </div>
      <div className="flex flex-col w-full p-4">
        <Link to={`/view-hotel/${hotel._id}`} target="_blank" className="flex justify-between">
          <div className="flex flex-col">
              <h3 className="text-2xl font-semibold">{hotel.hotelName}</h3>
            <div className="flex gap-2 items-center">
              <MdLocationOn className="text-green-700 text-xl" />
              <p className="text-lg font-normal">
              {hotel.address}
              {!(hotel.address.includes(hotel.city) || hotel.address.includes(hotel.state)) && (`, ${hotel.city}, ${hotel.state}`)}
              </p>
            </div>  
          </div>
        </Link>  
        ...
        <div className="flex justify-between mt-5 gap-10">
          ...
          <div className="flex gap-4">
            <a href={`/view-hotel/${hotel._id}`} target="_blank" className="border-2 border-black text-xl text-black p-1 px-5">View</a>
            <a href={`/book-hotel/${hotel._id}`} target="_blank" className="bg-green-700 p-2 px-5 text-white font-bold">Book</a>
          </div>
        </div>
      </div>
    </div>
